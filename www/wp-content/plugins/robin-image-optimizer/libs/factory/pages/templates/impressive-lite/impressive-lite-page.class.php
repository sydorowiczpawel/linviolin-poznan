...
if ( ! class_exists( 'Wbcr_FactoryPages445_ImpressiveLiteTemplate' ) ) {
	/**
	 * Class Wbcr_FactoryPages445_ImpressiveLiteTemplate
	 */
	abstract class Wbcr_FactoryPages445_ImpressiveLiteTemplate extends Wbcr_FactoryPages445_ImpressiveThemplate {
		...
		public function __construct( Wbcr_Factory446_Plugin $plugin ) {
			$this->menuIcon = FACTORY_PAGES_445_URL . '/templates/impressive-lite/assets/img/webcraftic-plugin-icon.png';
			//$allow_multisite = apply_filters('wbcr_factory_446_core_admin_allow_multisite', false);
			...
			$this->title_plugin_action_link = __( 'Settings', 'wbcr_factory_pages_445' );
		...
		public function assets( $scripts, $styles ) {
			...
			$this->styles->add( FACTORY_PAGES_445_URL . '/templates/impressive-lite/assets/css/impressive-lite.page.template.css' );
		}
		...
		public function getPluginTitle() {
			 ...
			 */
			$plugin_title = apply_filters( 'wbcr/factory/pages/impressive-lite/plugin_title', $this->plugin->getPluginTitle(), $this->plugin->getPluginName() );
		...
		public function getPageUrl() {
			 ...
			 */
			return apply_filters( 'wbcr/factory/pages/impressive-lite/base_url', $this->getBaseUrl(), $this->plugin->getPluginName(), $this->id );
		}
		...
		public function flushCacheAndRulesAction() {
			...
			if ( class_exists( 'WbcrFactoryClearfy237_Helpers' ) ) {
			 ...
			 */
			do_action( 'wbcr/factory/pages/impressive-lite/after_form_save', $this->plugin, $this );
			 ...
			 * @since 4.0.9 - изменено название экшена, без привязки к версии фреймворка
			 */
			$redirect_args = apply_filters( 'wbcr/factory/pages/impressive-lite/save_redirect_args', $redirect_args );
		...
		protected function warningNotice() {
			/*if( WP_CACHE ) {
				$this->printWarningNotice(__("It seems that a caching/performance plugin is active on this site. Please manually invalidate that plugin's cache after making any changes to the settings below.", 'wbcr_factory_pages_445'));
			}*/
		...
		private function showActionsNotice() {
			$notices = [
				[
					...
					'type'       => 'success',
					'message'    => __( 'The settings have been updated successfully!', 'wbcr_factory_pages_445' ) . ( WP_CACHE ? '<br>' . __( "It seems that a caching/performance plugin is active on this site. Please manually invalidate that plugin's cache after making any changes to the settings below.", 'wbcr_factory_pages_445' ) : '' )
				]
			];
			 ...
			 * @since 4.0.9 - изменено имя
			 */
			$notices = apply_filters( 'wbcr/factory/pages/impressive-lite/actions_notice', $notices, $this->plugin, $this->id );
		...
		public function setPageMenu() {
			global $factory_impressive_lite_page_menu;
			...
			$factory_impressive_lite_page_menu[ $this->getMenuScope() ][ $this->getResultId() ] = [
		...
		public function getMenuScope() {
			 ...
			 * @since 4.0.9 - добавлен
			 */
			return apply_filters( 'wbcr/factory/pages/impressive-lite/menu_scope', $this->plugin->getPluginName(), $this->plugin->getPluginName(), $this->id );
		}
		...
		public function getPageMenu() {
			...
			return $factory_impressive_lite_page_menu[ $this->getMenuScope() ];
		}
		...
		protected function showHeader() {
            ...
            <div class="wbcr-factory-page-header">
                ...
                <div class="wbcr-factory-control">
					<?php do_action( 'wbcr/factory/pages/impressive-lite/header', $this->plugin->getPluginName() ) ?>
					...
					<?php if ( $this->type == 'options' ): ?>
                        <input name="<?= $this->plugin->getPluginName() ?>_save_action"
                               ...
                               value="<?php _e( 'Save', 'wbcr_factory_pages_445' ); ?>">
		...
		protected function showOptions() {
			...
			if ( isset( $_POST[ $this->plugin->getPluginName() . '_save_action' ] ) ) {
				...
				if ( ! current_user_can( 'administrator' ) && ! current_user_can( $this->capabilitiy ) ) {
					wp_die( __( 'You do not have permission to edit page.', 'wbcr_factory_pages_445' ) );
				}
				 ...
				 * @since 4.0.9 - изменено имя
				 */
				do_action( 'wbcr/factory/pages/impressive-lite/before_form_save', $form, $this->plugin, $this );
				 ...
				 * @since 4.0.9 - изменено имя
				 */
				do_action( 'wbcr/factory/pages/impressive-lite/form_saved', $form, $this->plugin, $this );
            ...
            <div id="WBCR" class="wrap">
                <div class="wbcr-factory-pages-445-impressive-lite-page-template factory-bootstrap-446 factory-fontawesome-000">
